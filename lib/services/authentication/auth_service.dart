...
class AuthRepository {
  AuthRepository(
    this._accountRepo,
    this._tokenStore, {
    final DeviceFlowService? deviceFlowService,
  }) : _deviceFlowService = deviceFlowService ?? DeviceFlowService();

  final AccountRepository _accountRepo;
  final TokenStore _tokenStore;
  final DeviceFlowService _deviceFlowService;
  ...
  Future<String?> getAccessTokenForAccount(
    String username, {
    String? serverId,
  }) async {
  ...
  Future<String?> getAccessTokenFromDevice() async {
    try {
      ...
      AppLogger.error(
        'Failed to get access token from device',
        error: e,
        stackTrace: st,
      );
  ...
  Future<DeviceCodeResponse> getDeviceToken({
    ...
    return _deviceFlowService.requestDeviceCode(
      config: oauth,
  ...
  Future<AccessTokenResponse> getAccessToken({
    ...
    try {
      return await _deviceFlowService.pollAccessToken(
        config: oauth,
  ...
  Future<String?> getActiveAccount() async => _accountRepo.getActiveAccount();
  ...
  Future<void> updateAccountProfile({
    ...
    String? displayName,
    String? avatarUrl,
  }) async => _accountRepo.updateAccountProfile(
    nodeId: nodeId,
    serverId: serverId,
    username: username,
  ...
  Future<AccountModel> authenticateWithPAT({
    ...
    try {
      return await fetchViewerInfoWithToken(token, serverConfig: serverConfig);
    } catch (e) {
  ...
  Future<(ViewerUserPayload, String?)> _fetchUserData(
    String token, {
    String? apiBase,
  }) async {
    final dio = Dio(
      BaseOptions(baseUrl: apiBase ?? ServerConfig.gitHubDotCom.restBaseUrl),
    );
  ...
  Future<AccountModel> fetchViewerInfoWithToken(
    ...
    try {
      final (userData, grantedScopes) = await _fetchUserData(
        token,
