...
/// [fileContentProvider] and [directoryProvider] for the file's parent, then pops.
class FileEditSheet {
...
class _FileEditSheetBodyState extends State<_FileEditSheetBody> {
  ...
  void initState() {
    ...
    _commitMessageController =
        TextEditingController(text: 'Update ${widget.filePath}');
  }
  ...
  Future<void> _commit() async {
    ...
    if (message.isEmpty) {
      widget.ref.read(notificationServiceProvider).error(
    ...
    try {
      ...
      await widget.repoRef.gitDb(apiClient).commitFileChanges(
      ...
      final String parentPath = widget.filePath.contains('/')
          ? widget.filePath
              ...
              .join('/')
          : '';
      widget.ref.invalidate(fileContentProvider(
        (repo: widget.repoRef, branch: widget.branchRef, path: widget.filePath),
      ));
      widget.ref.invalidate(directoryProvider(
        (repo: widget.repoRef, branch: widget.branchRef, path: parentPath),
      ));
