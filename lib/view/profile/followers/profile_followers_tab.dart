...
/// Returns a [SliverListBody] for the followers list on a user profile.
TabBody createFollowersBody(
  WidgetRef ref,
  UserRef userRef, {
  ValueNotifier<String>? queryNotifier,
}) {
  return SliverListBody<UserFollowerEdge?>.textFilter(
    ...
    fetcher:
        ({String? after, int first = _pageSize, bool refresh = false}) async {
          final list = await ref
              ...
              .getUserFollowers(userRef.login, refresh: refresh, after: after);
    ...
    itemBuilder: (BuildContext context, UserFollowerEdge? item) {
      final UserFollowerNode? node = item?.node;
