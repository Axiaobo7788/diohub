...
/// )
/// ```
class RepositoryRadarChart extends ConsumerWidget {
  ...
  static const List<Color> _repositoryColors = <Color>[
  ...
  Widget build(final BuildContext context, final WidgetRef ref) {
    ...
    final List<ContributedRepository> displayRepos =
        repositories.take(maxRepositories).toList();
    ...
    double maxValue = 0;
    for (final ContributedRepository repo in displayRepos) {
      final int repoMax = <int>[
        repo.commitCount ?? 0,
        repo.issueCount ?? 0,
    ...
    final List<RadarDataSet> dataSets = <RadarDataSet>[];
    for (int i = 0; i < displayRepos.length; i++) {
      final ContributedRepository repo = displayRepos[i];
      final Color color = i < _repositoryColors.length
          ? _repositoryColors[i]
          : _repositoryColors[i % _repositoryColors.length];

      final List<RadarEntry> dataEntries = <RadarEntry>[
    ...
    final List<RadarEntry> maxValueEntries = List.generate(
      4,
      (final int index) => RadarEntry(value: maxValue),
    );
  ...
  Widget _buildLegend(
    ...
    return Wrap(
      ...
      children: List.generate(
        repos.length,
        (final int index) {
          final ContributedRepository repo = repos[index];
          final Color color = index < _repositoryColors.length
              ? _repositoryColors[index]
              : _repositoryColors[index % _repositoryColors.length];

          // Truncate repository name if too long
          final String repoName = repo.name;
          final String displayName = repoName.length > 15
