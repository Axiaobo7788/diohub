...
final selectedSearchTypeProvider = NotifierProvider.family<
    SelectedSearchTypeNotifier, SearchType, SearchScope>(
...
class SearchStateNotifier extends Notifier<SearchState> {
  ...
  @override
  SearchState build() {
    final viewerLogin = ref.watch(currentUserProvider).value?.login;
  ...
  void setRawFreeText(String text) {
    ...
    if (text.trim().isNotEmpty) {
      ref.read(searchSessionProvider.notifier).setSession(SearchSessionState(
  ...
  void addQualifier(QualifierExpression q) {
    state = state.copyWith(
      activeQualifiers: [...state.activeQualifiers, q],
    );
  }
  ...
  void applyPreset(NavigationPreset preset) {
    ...
    final q = _registry.parseToken(qs);
    if (q != null) {
      state = state.copyWith(activeQualifiers: [q]);
    }
  }
