...
def fix_imports_in_package():
    """Fix imports in packages/diohub_models/ files to use diohub_models package for moved files"""
    
    pkg_models_dir = os.path.join(workspace, 'packages', 'diohub_models', 'lib', 'models')
    
    for root, dirs, files in os.walk(pkg_models_dir):
        for file in files:
