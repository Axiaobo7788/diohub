...
class ExtendedNestedScrollView extends StatefulWidget {
  ...
  /// Like [sliverOverlapAbsorberHandleFor], but returns `null` instead of
  /// asserting when there is no [NestedScrollView] ancestor.
  static SliverOverlapAbsorberHandle? maybeSliverOverlapAbsorberHandleFor(
      BuildContext context) {
    final _InheritedNestedScrollView? target = context
        .dependOnInheritedWidgetOfExactType<_InheritedNestedScrollView>();
    return target?.state._absorberHandle;
  ...
  _NestedScrollMetrics _getMetrics(
    ...
    if (innerPosition.pixels == innerPosition.minScrollExtent) {
      ...
      if ((velocity > 0.0) &&
        ...
        extra = math.max(
            0.0, _outerPosition!.maxScrollExtent - _outerPosition!.pixels);
        minRange = pixels;
        ...
        extra = math.max(
            0.0, _outerPosition!.pixels - _outerPosition!.minScrollExtent);
